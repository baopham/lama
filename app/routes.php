...
Route::group(array('prefix' => 'api/v1', 'before' => 'xhr|xsrf'), function () {
    ...
    ))->where('id', '[0-9]+')->before('hasAccessAndIsOwner:users.show');
    Route::put('users/{id}', array(
        'as' => 'users.edit',
        'uses' => 'App\Controllers\UsersController@edit'
    ))->where('id', '[0-9]+')->before('hasAccess:users');
    Route::put('users/{id}/account', array(
        'as' => 'users.account',
        'uses' => 'App\Controllers\UsersController@account'
    ))->where('id', '[0-9]+')->before('hasAccessAndIsOwner:users.account');
    Route::put('users/{id}/password', array(
        'as' => 'users.password',
        'uses' => 'App\Controllers\UsersController@password'
    ))->where('id', '[0-9]+')->before('hasAccessAndIsOwner:users.password');
    Route::put('users/{id}/suspend', array(
        'as' => 'users.suspend',
        'uses' => 'App\Controllers\UsersController@suspend'
    ))->where('id', '[0-9]+')->before('hasAccess:users');
    Route::put('users/{id}/unsuspend', array(
        'as' => 'users.unsuspend',
        'uses' => 'App\Controllers\UsersController@unsuspend'
    ))->where('id', '[0-9]+')->before('hasAccess:users');
    Route::put('users/{id}/ban', array(
        'as' => 'users.ban',
...
Route::get('users/{id}/activate/{code}', array(
    'as' => 'users.activate',
    'uses' => 'App\Controllers\UsersController@activate'
))->where('id', '[0-9]+');
Route::get('users/{id}/reset/{code}', array(
    'as' => 'users.reset',
